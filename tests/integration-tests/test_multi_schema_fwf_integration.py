...
class TestMultiSchemaFwfS3Integration:
    ...
    def test_custom_multi_schema_fwf_s3_upload(self, s3_client, s3_config, cleanup_s3_objects):
        ...
        conditional_schemas = [
            ...
            FwfConditionalSchema(
                ...
                [
                    ...
                    FwfFieldSpec(
                        "customer_id", 12, 8, align="right", pad="0", parquet_type="int64"
                    ),
                    FwfFieldSpec("order_date", 20, 8, parquet_type="string"),
                    FwfFieldSpec(
                        "total_amount", 28, 12, align="right", pad="0", parquet_type="int64"
                    ),
            ...
            FwfConditionalSchema(
                ...
                [
                    ...
                    FwfFieldSpec(
                        "unit_price", 50, 10, align="right", pad="0", parquet_type="int64"
                    ),
                ],
            ),
        ]
        ...
        try:
            ...
            try:
                ...
                test_key = (
                    f"forklift/integration-test/custom-multi-schema-{int(time.time())}.parquet"
                )
                s3_path = f"s3://{s3_config['test_bucket']}/{test_key}"
                cleanup_s3_objects.append(s3_path)
                ...
                assert (
                    s3_client.get_size(s3_path) > 0
                ), "Custom multi-schema parquet should have content"
                ...
                try:
                    ...
                    assert (
                        downloaded_df["record_type"].nunique() == 3
                    ), "Should preserve all record types"
    ...
    def test_hierarchical_multi_schema_fwf_s3_upload(
        ...
        conditional_schemas = [
            ...
            FwfConditionalSchema(
                ...
                [
                    ...
                    FwfFieldSpec(
                        "customer_id", 10, 8, align="right", pad="0", parquet_type="int64"
                    ),
            ...
            FwfConditionalSchema(
                ...
                [
                    ...
                    FwfFieldSpec("invoice_id", 2, 8, align="right", pad="0", parquet_type="int64"),
                    FwfFieldSpec(
                        "line_number", 10, 3, align="right", pad="0", parquet_type="int32"
                    ),
                    ...
                    FwfFieldSpec(
                        "unit_price", 30, 10, align="right", pad="0", parquet_type="int64"
                    ),
                    FwfFieldSpec(
                        "line_total", 40, 10, align="right", pad="0", parquet_type="int64"
                    ),
                ],
            ),
            # Tax details
            FwfConditionalSchema(
                ...
                [
                    ...
                    FwfFieldSpec(
                        "tax_amount", 23, 10, align="right", pad="0", parquet_type="int64"
                    ),
        ...
        try:
            ...
            assert (
                invoice_ids == line_item_invoice_ids
            ), "Line items should reference valid invoices"
            ...
            try:
                ...
                try:
                    ...
                    assert (
                        d_invoice_ids == d_line_invoice_ids
                    ), "Downloaded line items should reference valid invoices"
                    assert (
                        d_invoice_ids == d_tax_invoice_ids
                    ), "Downloaded tax records should reference valid invoices"
...
class TestMultiSchemaFwfLocal:
    ...
    def test_large_multi_schema_processing(self):
        ...
        try:
            ...
            print(
                f"Processed {table.num_rows} multi-schema records in {processing_time:.2f} seconds"
            )
            assert (
                processing_time < 15.0
            ), f"Processing should be under 15 seconds, took {processing_time:.2f}s"
