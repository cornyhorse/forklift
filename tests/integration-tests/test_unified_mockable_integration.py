...
class TestMockableFileUploadIntegration:
    ...
    def test_csv_to_parquet_s3_upload_mockable(
        self, s3_client, s3_test_bucket, cleanup_s3_objects, s3_mock_conditional, test_files_dir
    ):
        ...
        try:
            ...
            try:
                ...
                if parquet_files:
                    ...
                    data_file = next(
                        (f for f in parquet_files if "data.parquet" in f), parquet_files[0]
                    )
    ...
    def test_fwf_to_parquet_s3_upload_mockable(
    ...
    def test_multi_schema_fwf_to_parquet_s3_upload_mockable(
        ...
        try:
            ...
            try:
                ...
                # Upload to S3
                test_key = (
                    f"forklift/integration-test/multi-fwf-mockable-{int(time.time())}.parquet"
                )
                s3_path = f"s3://{s3_test_bucket}/{test_key}"
                cleanup_s3_objects.append(s3_path)
    ...
    def test_batch_file_processing_s3_upload_mockable(
        self, s3_client, s3_test_bucket, cleanup_s3_objects, s3_mock_conditional
    ):
            ...
            for file_type, file_path in test_files:
                ...
                if file_type == "csv":
                    ...
                    if results.output_files:
                        upload_file = next(
                            (f for f in results.output_files if "data.parquet" in f),
                ...
                assert s3_client.exists(
                    s3_path
                ), f"{file_type.upper()} parquet file should exist in S3"
