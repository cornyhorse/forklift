...
class TestExcelS3MockableIntegration:
    ...
    def test_excel_multi_sheet_s3_upload_mockable(self, s3_client, s3_bucket, cleanup_s3_objects):
        ...
        for i, (sheet_name, table) in enumerate(results):
            if table.num_rows > 0:  # Only upload if table has data
                ...
                s3_path = S3Path(f"s3://{s3_bucket}/test-uploads/excel-multi-mockable-{sheet_name}-{timestamp}-{i}.parquet")
    ...
    def test_excel_1904_dates_s3_upload_mockable(self, s3_client, s3_bucket, cleanup_s3_objects):
        ...
        try:
            ...
            if results and results[0][1].num_rows > 0:  # Check if table has data
                ...
                # Upload to S3
                timestamp = int(time.time())
                s3_path = S3Path(f"s3://{s3_bucket}/test-uploads/excel-1904-dates-{timestamp}.parquet")
    ...
    def test_excel_headerless_sheet_s3_upload_mockable(self, s3_client, s3_bucket, cleanup_s3_objects):
        ...
        try:
            ...
            if results and results[0][1].num_rows > 0:  # Check if table has data
                ...
                # Upload to S3
                timestamp = int(time.time())
                s3_path = S3Path(f"s3://{s3_bucket}/test-uploads/excel-headerless-{timestamp}.parquet")
