...
@pytest.mark.s3
class TestFwfS3MockableIntegration:
    """FWF integration tests that support both mocked and real S3."""
    ...
    def test_large_fwf_file_streaming_to_s3_mockable(
        ...
        # Create large test data (1000 records for faster testing in mock mode)
        test_size = 1000
        ...
        with tempfile.NamedTemporaryFile(
            ...
            for i in range(test_size):
                line = f"{i:08d}{'Data_' + str(i):20}{(i * 100):010d}\n"
                tmp_file.write(line)

        try:
            handler = FwfInputHandler(config)

            # Create Arrow table from large FWF file
            table = handler.create_arrow_table(fwf_path)
            ...
            try:
                ...
                # Verify upload succeeded
                assert s3_client.exists(s3_path), "Large parquet file should exist in S3"

                # Additional verification for real S3 only
                mock_session, mock_client = s3_mock_conditional
                if mock_session is None:  # Real S3 only
                    file_size = s3_client.get_size(s3_path)
                    assert file_size > 10000, "Large parquet file should be substantial"
