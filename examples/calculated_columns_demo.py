...
def demo_business_logic_expressions():
    ...
    schema = pa.schema([pa.field(name, array.type) for name, array in zip(employee_data.keys(), arrays)])
    batch = pa.RecordBatch.from_arrays(arrays, schema=schema)
...
def demo_data_quality_calculations():
    ...
    quality_data = {
        ...
        "email": ["john@company.com", "jane@invalid", "missing", "bob@company.com", "alice@company.com"],
    ...
    schema = pa.schema([pa.field(name, array.type) for name, array in zip(quality_data.keys(), arrays)])
    batch = pa.RecordBatch.from_arrays(arrays, schema=schema)
...
def demo_schema_driven_processing():
    ...
    if importer.has_calculated_columns():
        ...
        schema = pa.schema([pa.field(name, array.type) for name, array in zip(sample_data.keys(), arrays)])
        batch = pa.RecordBatch.from_arrays(arrays, schema=schema)
        ...
        print(f"• Added columns: {[col for col in result_batch.schema.names if col not in batch.schema.names]}")
...
def demo_advanced_expressions():
    ...
    schema = pa.schema([pa.field(name, array.type) for name, array in zip(financial_data.keys(), arrays)])
    batch = pa.RecordBatch.from_arrays(arrays, schema=schema)
