...
class TestSchemaValidationMode:
    ...
    def test_schema_validation_mode_values(self):
        """Test SchemaValidationMode enum values."""
        assert SchemaValidationMode.STRICT.value == "strict"
        assert SchemaValidationMode.PERMISSIVE.value == "permissive"
        assert SchemaValidationMode.COERCE.value == "coerce"


class TestNullabilityMode:
    """Test NullabilityMode enum."""

    def test_nullability_mode_values(self):
        """Test NullabilityMode enum values."""
        assert NullabilityMode.ERROR.value == "error"
        assert NullabilityMode.WARNING.value == "warning"
        assert NullabilityMode.IGNORE.value == "ignore"
...
class TestColumnSchema:
    ...
    def test_column_schema_defaults(self):
        """Test ColumnSchema with default values."""
        col_schema = ColumnSchema(name="test", data_type="string")
        assert col_schema.name == "test"
        assert col_schema.data_type == "string"
        assert col_schema.nullable is True
        assert col_schema.constraints == {}
        assert col_schema.description is None

    def test_column_schema_custom_values(self):
        """Test ColumnSchema with custom values."""
        ...
        assert col_schema.name == "id"
        assert col_schema.data_type == "int64"
        assert col_schema.nullable is False
        assert col_schema.constraints == constraints
        assert col_schema.description == "Primary key"

    def test_column_schema_post_init_none_constraints(self):
        """Test ColumnSchema __post_init__ with None constraints."""
        col_schema = ColumnSchema(name="test", data_type="string", constraints=None)
        assert col_schema.constraints == {}


class TestSchemaValidatorConfig:
    """Test SchemaValidatorConfig dataclass."""

    def test_schema_validator_config_defaults(self):
        """Test SchemaValidatorConfig with default values."""
        config = SchemaValidatorConfig()
        assert config.validation_mode == SchemaValidationMode.STRICT
        assert config.nullability_mode == NullabilityMode.ERROR
        assert config.allow_type_coercion is False
        assert config.check_column_order is False
        assert config.case_sensitive is True
        assert config.extra_columns_allowed is False
        assert config.max_null_percentage is None
        assert config.min_row_count is None
        assert config.max_row_count is None

    def test_schema_validator_config_custom_values(self):
        """Test SchemaValidatorConfig with custom values."""
        config = SchemaValidatorConfig(
            validation_mode=SchemaValidationMode.PERMISSIVE,
            nullability_mode=NullabilityMode.WARNING,
        ...
        assert config.validation_mode == SchemaValidationMode.PERMISSIVE
        assert config.nullability_mode == NullabilityMode.WARNING
        assert config.allow_type_coercion is True
        assert config.check_column_order is True
        assert config.case_sensitive is False
        assert config.extra_columns_allowed is True
...
class TestSchemaValidatorInitialization:
    ...
    def test_init_with_arrow_schema(self):
        """Test initialization with PyArrow schema."""
        schema = pa.schema([
        ...
        assert isinstance(validator.schema_definition, dict)
        assert "columns" in validator.schema_definition
        assert len(validator.expected_columns) == 2
        assert "id" in validator.expected_columns
        assert "name" in validator.expected_columns
    ...
    def test_init_with_dict_schema(self):
        ...
        assert validator.schema_definition == schema_dict
        assert len(validator.expected_columns) == 2
        assert validator.expected_columns["id"].nullable is False
        assert validator.expected_columns["name"].nullable is True
    ...
    def test_init_with_strict_mode_true(self):
        ...
        assert validator.config.validation_mode == SchemaValidationMode.STRICT
        assert validator.config.extra_columns_allowed is False
    ...
    def test_init_with_strict_mode_false(self):
        ...
        assert validator.config.extra_columns_allowed is True

    def test_init_with_custom_config(self):
        """Test initialization with custom config."""
        schema = pa.schema([pa.field("id", pa.int64())])
        config = SchemaValidatorConfig(
            validation_mode=SchemaValidationMode.COERCE,
            nullability_mode=NullabilityMode.WARNING
        )
        validator = SchemaValidator(schema, config=config)

        assert validator.config.validation_mode == SchemaValidationMode.COERCE
        assert validator.config.nullability_mode == NullabilityMode.WARNING

    def test_init_with_empty_dict(self):
        """Test initialization with empty dictionary."""
        validator = SchemaValidator({})
        assert len(validator.expected_columns) == 0
        assert validator.schema is None

    def test_init_with_none_schema_definition(self):
        """Test initialization with None schema definition."""
        validator = SchemaValidator(None)
        assert len(validator.expected_columns) == 0
        assert validator.schema is None
        # Handle None case properly
        assert validator.schema_definition is None


class TestSchemaValidatorHelperMethods:
    """Test SchemaValidator helper methods."""

    def test_convert_arrow_schema_to_dict(self):
        """Test _convert_arrow_schema_to_dict method."""
        schema = pa.schema([
            pa.field("id", pa.int64(), nullable=False),
            pa.field("name", pa.string()),
        ...
        validator = SchemaValidator({})
        result = validator._convert_arrow_schema_to_dict(schema)
    ...
    def test_convert_dict_to_arrow_schema(self):
        """Test _convert_dict_to_arrow_schema method."""
        schema_dict = {
            "columns": [
                {"name": "id", "type": "int64", "nullable": False},
                {"name": "name", "type": "string", "nullable": True}
        ...
        validator = SchemaValidator({})
        result = validator._convert_dict_to_arrow_schema(schema_dict)
    ...
    def test_convert_dict_to_arrow_schema_no_columns(self):
        """Test _convert_dict_to_arrow_schema with no columns."""
        validator = SchemaValidator({})
        result = validator._convert_dict_to_arrow_schema({"metadata": {}})
        assert result is None

    def test_convert_dict_to_arrow_schema_empty_columns(self):
        """Test _convert_dict_to_arrow_schema with empty columns."""
        validator = SchemaValidator({})
        result = validator._convert_dict_to_arrow_schema({"columns": []})
        assert result is None

    def test_string_to_arrow_type_mapping(self):
        """Test _string_to_arrow_type method with various types."""
        validator = SchemaValidator({})

        # Test integer types
        assert validator._string_to_arrow_type("int") == pa.int64()
        assert validator._string_to_arrow_type("integer") == pa.int64()
        assert validator._string_to_arrow_type("int64") == pa.int64()
        assert validator._string_to_arrow_type("int32") == pa.int32()

        # Test float types
        assert validator._string_to_arrow_type("float") == pa.float64()
        assert validator._string_to_arrow_type("double") == pa.float64()
        assert validator._string_to_arrow_type("float64") == pa.float64()
        assert validator._string_to_arrow_type("float32") == pa.float32()

        # Test string types
        assert validator._string_to_arrow_type("string") == pa.string()
        assert validator._string_to_arrow_type("str") == pa.string()
        assert validator._string_to_arrow_type("text") == pa.string()

        # Test boolean types
        assert validator._string_to_arrow_type("bool") == pa.bool_()
        assert validator._string_to_arrow_type("boolean") == pa.bool_()

        # Test date types
        assert validator._string_to_arrow_type("date") == pa.date32()
        assert validator._string_to_arrow_type("datetime") == pa.timestamp('us')
        assert validator._string_to_arrow_type("timestamp") == pa.timestamp('us')

        # Test unknown type defaults to string
        assert validator._string_to_arrow_type("unknown") == pa.string()

    def test_parse_schema_definition(self):
        """Test _parse_schema_definition method."""
        schema_dict = {
            "columns": [
                {
                    "name": "id",
        ...
        assert columns["id"].constraints == {"min": 1}
        assert columns["id"].description == "Primary key"

        assert columns["name"].name == "name"
        assert columns["name"].data_type == "string"
        assert columns["name"].nullable is True
        assert columns["name"].constraints == {}
        assert columns["name"].description is None

    def test_parse_schema_definition_invalid_column(self):
        """Test _parse_schema_definition with invalid column definition."""
        schema_dict = {
            "columns": [
                "invalid_column_def",  # Not a dict
                {"name": "valid", "type": "string"}
            ]
        }
        validator = SchemaValidator(schema_dict)
        # Should only parse valid column
        assert len(validator.expected_columns) == 1
        assert "valid" in validator.expected_columns


class TestSchemaValidatorBatchStructure:
    """Test batch structure validation."""

    def test_validate_batch_structure_none_batch(self):
        """Test validation with None batch."""
        validator = SchemaValidator({})
        results = validator._validate_batch_structure(None)

        assert len(results) == 1
        assert results[0].is_valid is False
        assert results[0].error_code == "NULL_BATCH"
        assert "Batch is None" in results[0].error_message

    def test_validate_batch_structure_empty_batch_with_min_rows(self):
        """Test validation with empty batch and minimum row requirement."""
        config = SchemaValidatorConfig(min_row_count=1)
        validator = SchemaValidator({}, config=config)

        # Create a proper empty batch with correct schema
        schema = pa.schema([pa.field("id", pa.int64())])
        batch = pa.RecordBatch.from_arrays([pa.array([], type=pa.int64())], schema=schema)

        results = validator._validate_batch_structure(batch)

        assert len(results) == 1
        assert results[0].is_valid is False
        assert results[0].error_code == "EMPTY_BATCH"

    def test_validate_batch_structure_valid(self):
        """Test validation with valid batch."""
        validator = SchemaValidator({})
