...
class TestExcelImporter:
        ...
        with patch("forklift.inputs.excel.ExcelInputHandler") as mock_handler_class:
            ...
            with patch("pyarrow.parquet.write_table") as mock_write:
                result = ExcelImporter.import_excel(
                    input_path=sample_excel_file, output_path=output_directory
                )
    ...
    def test_import_excel_with_schema(
        self, sample_excel_file, output_directory, sample_schema_file
    ):
        """Test Excel import with schema file."""
        with patch(
            "forklift.schema.excel_schema_importer.ExcelSchemaImporter"
    ...
    def test_import_excel_file_not_found(self, output_directory):
        ...
        with pytest.raises(FileNotFoundError):
            ExcelImporter.import_excel(input_path=non_existent_file, output_path=output_directory)
    ...
    def test_import_excel_with_kwargs(self, sample_excel_file, output_directory):
        ...
        with patch("forklift.inputs.excel.ExcelInputHandler") as mock_handler_class:
            with patch(
                "forklift.engine.importers.excel_importer.ExcelImporter._create_default_excel_config"
            ) as mock_config:
                ...
                with tempfile.NamedTemporaryFile(suffix=".xlsx") as tmp_file:
                    ...
                    with patch("pyarrow.parquet.write_table") as mock_write:
                        with patch.object(Path, "exists", return_value=True):
                            ExcelImporter.import_excel(
                                input_path=Path(tmp_file.name), output_path=output_dir
                            )
